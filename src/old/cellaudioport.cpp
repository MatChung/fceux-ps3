...
static size_t drain_fifo(void *cb_data, float **data)
{
	...
	union
	{
	  uint32_t utmp[CELL_AUDIO_BLOCK_SAMPLES];
	  int16_t tmp[CELL_AUDIO_BLOCK_SAMPLES * 2];
	} u;

	if (fifo_read_avail(aud->buffer) >= sizeof(u))
	{
		pthread_mutex_lock(&aud->lock);
		fifo_read(aud->buffer, &u, sizeof(u));
		pthread_mutex_unlock(&aud->lock);
		//array_to_float(aud->tmp_data, tmp, CELL_AUDIO_BLOCK_SAMPLES * AUDIO_CHANNELS);

		/*for (int i = 0; i < (int)CELL_AUDIO_BLOCK_SAMPLES * AUDIO_CHANNELS; i++)
		{
		  //sample = ((float)(tmp[i] & 0xFFFF)) / 0x7FFF;
		  //aud->tmp_data[i] = sample | sample << 16;
		  aud->tmp_data[i] = (float)tmp[i] / 0x7FFF;
		}*/

		for (int i = 0; i < (int)CELL_AUDIO_BLOCK_SAMPLES; i++)
		{
		  aud->tmp_data[2*i] = (float)u.tmp[2 * i + 1]/0x7FFF;
		  aud->tmp_data[2*i + 1] = (float)u.tmp[2 * i + 1]/0x7FFF;
		}
	}
	else
	{
