...
UnZipBuffer (unsigned char *outbuffer)
{
        ...
        FCEU_printf("Loading... %d/%d", 0, pkzip.uncompressedSize);
...
GetFirstZipFilename ()
{
        ...
        // read start of ZIP
        if(LoadFile (tempbuffer, filepath, ZIPCHUNK, false) < 35)
                return NULL;
        ...
        {
                FCEU_PrintError("Error - Invalid ZIP file!");
                return NULL;
        }
...
static SZ_RESULT SzFileReadImp(void *object, void **buffer, size_t maxRequiredSize, size_t *processedSize)
{
        ...
        *processedSize = sizeread;
        ...
        if(sizeread > 1024) // only show progress for large reads
                ...
                FCEU_printf("Loading large file... %d", s->pos);
